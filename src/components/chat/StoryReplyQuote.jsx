...
export default function StoryReplyQuote({
  ...
  const content = (
    <>
      <p className="text-xs font-semibold truncate mb-1 text-[var(--story-quote-label)]">{label}</p>
      <p
        className={`text-[13px] leading-snug line-clamp-3 whitespace-pre-wrap break-words ${
          removed ? 'text-[var(--story-quote-label)] italic' : 'text-[var(--story-quote-text)]'
        }`}
      >
        {snippet}
      </p>
  ...
  const boxClass = `relative overflow-hidden rounded-[var(--ios-radius-md)] px-3 py-2.5 border border-[var(--ios-hairline)] ${colorClass} ${
